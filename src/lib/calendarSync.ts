...
// Jádro synchronizace plánu úkolu do Google kalendářů „{jméno} - KRONOS".
// POUZE server. Volá ho server action (karta, Můj den) i MCP server —
// úkol se čte klientem volajícího, takže RLS hlídá práva jako všude jinde.
...
/** Propíše plánované okno úkolu do kalendářů jeho řešitelů (bez okna
    události maže). `userClient` je klient volajícího — RLS ověří přístup. */
export async function syncTaskCalendarCore(
  ...
  // úkol čteme pod volajícím — RLS ověří, že na něj smí
  ...
  try {
    for (const target of targets) {
      ...
      // kalendář — jednou založit, pak používat cachované id
      let calendarId = (
        await admin
